...
public class PurchaseRepository : IPurchaseRepository
{
    ...
    public async Task<Purchase> GetPurchaseCreditCardAsync(int? id)
    {
        return await _purchaseRepository.Purchases.Include(p => p.CreditCard).SingleOrDefaultAsync(p => p.Id == id);
    }

    public async Task<IEnumerable<Purchase>> GetPurchasesAsync(int creditCardId)
    {
        return await _purchaseRepository.Purchases.Where(p => p.CreditCardId == creditCardId).ToListAsync();
    }

    public async Task<Purchase> RemoveAsync(Purchase purchase)
    {
